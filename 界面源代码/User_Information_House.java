...
public class User_Information_House extends Log_In implements ActionListener {

	JFrame f = new JFrame("个人房屋信息");
	JButton b2 = new JButton("查询");
	JButton b3 = new JButton("返回");
	String[] cloum = { "房号", "使用面积","现使用人ID","上一使用人ID","最早使用人ID","阳台面积"};
		...
		if (b2.equals(e.getSource())) {// 查询个人工资
			...
			try {
				...
				if (b2.equals(e.getSource())) {
					...
					rs = sql.executeQuery("select  * from  HousingSituation  where  Hno=(select Hno from Worker where Worker.Dno='"+t1+"')");
					while (rs.next()) {
                        for(int i=1;i<=6;i++)
                        {
