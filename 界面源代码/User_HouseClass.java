...
public class User_HouseClass extends Log_In implements ActionListener {

	JFrame f = new JFrame("住房级别信息");
		...
		if (b2.equals(e.getSource())) {// 查询个人工资
			...
			try {
				...
				if (b2.equals(e.getSource())) {
					...
					rs = sql.executeQuery("select * from  Class  where  Class=(select Class from HousingLevel where APosition= ( select APosition from Worker where Worker.Wno='"+t1+"') and PTPosition=( select APosition from Worker where Worker.Wno='"+t1+"'));");
					while (rs.next()) {
                        for(int i=1;i<=3;i++)
                        {
